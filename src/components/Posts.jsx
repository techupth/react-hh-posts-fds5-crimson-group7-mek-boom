...
function Posts() {
  const [postList, setPostlist] = useState(postListDefalt); //สร้าง state

  const likePost = (postIndex) => {
    //สร้าง fn. และ paramiter คือค่าที่รับมากจาก onclick(index)
    const newPostlist = [...postList]; //โคลน array state เพืิ่อไว้ re-render(Like)
  ...
  const disLikePost = (postIndex) => {
    ...
    if (newPostlist[postIndex].likes > 0) {
      //ใช้ condition ถ้า ​​​like น้อยกว่า 0  เท่านั้น ถึงจะสามารถกด dislike แล้วจะทำงงานได้
      newPostlist[postIndex].likes = newPostlist[postIndex].likes - 1;
      setPostlist(newPostlist);
    }
  };
    ...
    <div class="app-wrapper">
      ...
      <div class="post-list">
        {postList.map((post, index) => {
          // เอา postlist มา map เพื่อสร้าง object ใหม่ไว้ re-render
          return (
            <div key={index} class="post-item">
              <div class="post-header">
                ...
                {/*  เช่น {post.title} และอันนอื่นที่คล้ายกัน จะเป็นการเข้าถึงข่้อมูลของ variable ที่ .mapไป */}
              </div>
              ...
              <div class="post-actions">
                <button
                  class="like-button"
                  onClick={() => {
                    likePost(index); //ใช้ fn แล้วใช้ argurment เป็น index เพื่อใช่เป็น para,iter ใน fn. like post
                  }}
                >
                  Like
                </button>
                <button
                  ...
                  onClick={() => {
                    disLikePost(index); //ใช้ fn แล้วใช้ argurment เป็น index เพื่อใช่เป็น para,iter ใน fn. like post
                  }}
                >
                  Dislike
                </button>
